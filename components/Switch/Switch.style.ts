...
const useStyle = ({ variant, enabled, disabled }: SwitchProps) => {
  ...
  const switchClasses = classNames(
    'inline-block bg-white h-5 w-5 rounded-full transforme transition-transform duration-200 ease-in-out',
    enabled ? 'translate-x-[2.1rem]' : 'traslate-x-1',
  )
